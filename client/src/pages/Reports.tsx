...
import { getOtherIncome, getPayments, OtherIncomeSummary, PaymentRecord } from "@/lib/DatabaseService";
...
export default function Reports() {
  ...
  const summary = useMemo(() => {
    ...
    const memberCollected = filteredPayments.reduce((sum, payment) => sum + payment.amount, 0);
    const otherCollected = filteredOther.reduce((sum, row) => sum + row.amount, 0);
    ...
    return { expectedDues, memberCollected, otherCollected, totalCollected: memberCollected + otherCollected, outstanding, paidCount, total: filteredDues.length, collectionPercent: expectedDues ? Math.round((memberCollected / expectedDues) * 100) : 0 };
  }, [filteredDues, filteredPayments, filteredOther]);
  ...
  const exportCSV = () => {
    ...
    const content = `${csvSection(exportMemberView === "dues" ? "MEMBER MONTHLY DUES" : "MEMBER PAYMENTS", memberHeaders, memberRows)}\n\n${csvSection("OTHER COLLECTIONS", ["Category", "Date", "Amount", "Payment Mode", "Notes"], otherRows)}\n\n${csvSection("OTHER TOTALS", ["Friday Total", "Room Rent Total", "Other Total"], [[otherIncome.fridayTotal, otherIncome.roomRentTotal, otherIncome.otherTotal]])}`;
  ...
  const exportXLSX = () => {
    ...
    const otherSheet = XLSX.utils.aoa_to_sheet([["OTHER COLLECTIONS"], [], ["Friday Collections"], ["Date", "Amount", "Payment Mode", "Notes"], ...otherIncome.fridayCollections.map((row) => [row.collectionDate, row.amount, row.paymentMode === "account" ? "Account" : "Cash", row.notes]), [], ["Friday Total", otherIncome.fridayTotal], [], ["Room Rent"], ["Date", "Amount", "Payment Mode", "Notes"], ...otherIncome.roomRents.map((row) => [row.rentDate, row.amount, row.paymentMode === "account" ? "Account" : "Cash", row.notes]), [], ["Room Rent Total", otherIncome.roomRentTotal], ["Other Total", otherIncome.otherTotal]]);
  ...
  const exportPDF = () => {
    const doc = new jsPDF(); doc.setFontSize(16); doc.text(`${settings.name} - ${exportMemberView === "dues" ? "Monthly Dues" : "Actual Payments"} Report`, 14, 18); doc.setFontSize(10); doc.setTextColor(120); doc.text(`Generated ${new Date().toLocaleString()}`, 14, 25);
    autoTable(doc, exportMemberView === "dues" ? { startY: 32, head: [["Member", "Due", "Received", "Pending", "Status", "Payment Date", "Voucher", "Period"]], body: filteredDues.map((member) => [member.name, formatPdfAmount(member.amount), formatPdfAmount(member.amount_paid), formatPdfAmount(member.amount_pending), member.status, member.payment_date ?? "—", member.voucher_number ?? "—", `${MONTHS[member.month - 1]} ${member.year}`]), headStyles: { fillColor: [20, 120, 90] }, styles: { fontSize: 8 } } : { startY: 32, head: [["Date", "Voucher", "Member", "Received", "Mode", "Allocated Months"]], body: filteredPayments.map((payment) => [payment.paymentDate, payment.voucherNumber, memberById.get(payment.memberId)?.name ?? "—", formatPdfAmount(payment.amount), payment.paymentMethod === "account" ? "Account" : "Cash", allocationsLabel(payment)]), headStyles: { fillColor: [20, 120, 90] }, styles: { fontSize: 8 } });
    ...
    const rentEnd = (doc as jsPDF & { lastAutoTable?: { finalY: number } }).lastAutoTable?.finalY ?? fridayEnd + 30; doc.setTextColor(20); doc.setFontSize(11); doc.text(`Room Rent Total: ${formatPdfAmount(otherIncome.roomRentTotal)}`, 14, rentEnd + 8); doc.setFontSize(13); doc.text(`Other Total: ${formatPdfAmount(otherIncome.otherTotal)}`, 14, rentEnd + 16); doc.save(`report-${Date.now()}.pdf`); toast.success("PDF exported");
  };

  return <AppShell title="Reports" subtitle="Member dues, actual payments, allocations, Friday collections, and room-rent reports">
    <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-5"><StatCard label="Member Collection" value={`${c}${summary.memberCollected.toLocaleString()}`} icon={Wallet} tone="success" /><StatCard label="Other Collection" value={`${c}${summary.otherCollected.toLocaleString()}`} icon={Landmark} tone="accent" /><StatCard label="Total Collection" value={`${c}${summary.totalCollected.toLocaleString()}`} icon={Building2} tone="primary" /><StatCard label="Outstanding" value={`${c}${summary.outstanding.toLocaleString()}`} icon={AlertTriangle} tone="destructive" /><StatCard label="Expected Dues" value={`${c}${summary.expectedDues.toLocaleString()}`} icon={Users} tone="accent" /></div>
