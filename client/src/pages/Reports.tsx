...
import { getLedgerDashboardSummary, getOtherIncome, getPayments, LedgerDashboardSummary, OtherIncomeSummary, PaymentRecord } from "@/lib/DatabaseService";
...
const emptyAccounting: LedgerDashboardSummary = { total: 0, paid: 0, unpaid: 0, pending: 0, partial: 0, expectedDues: 0, monthlyCollection: 0, yearlyCollection: 0, outstanding: 0, cashReceived: 0, accountReceived: 0, collectionPercent: 0, memberMonthlyCollection: 0, memberYearlyCollection: 0, fridayCollection: 0, roomRentCollection: 0, otherCollection: 0, otherCashReceived: 0, otherAccountReceived: 0, totalCollection: 0, yearlyFridayCollection: 0, yearlyRoomRentCollection: 0, yearlyOtherCollection: 0, yearlyTotalCollection: 0 };
...
export default function Reports() {
  ...
  const exportCSV = () => {
    ...
    const accountingRows = [[summary.memberCollected, summary.fridayCollected, summary.roomRentCollected, summary.otherCollected, summary.totalCollected, summary.yearlyTotal, accounting.cashReceived, accounting.accountReceived]];
    const content = `${csvSection("ACCOUNTING SUMMARY", ["Member Payments", "Friday Collection", "Room Rent", "Other Collection", "Monthly Collection", "Yearly Total", "Cash Received", "Account Received"], accountingRows)}\n\n${csvSection(exportMemberView === "dues" ? "MEMBER MONTHLY DUES" : "MEMBER PAYMENTS", memberHeaders, memberRows)}\n\n${csvSection("OTHER COLLECTIONS", ["Category", "Date", "Amount", "Payment Mode", "Notes"], otherRows)}\n\n${csvSection("OTHER TOTALS", ["Friday Total", "Room Rent Total", "Other Total", "Monthly Collection"], [[summary.fridayCollected, summary.roomRentCollected, summary.otherCollected, summary.totalCollected]])}`;
