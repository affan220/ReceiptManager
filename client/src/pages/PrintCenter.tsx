...
import { getMemberPayments, getOtherIncome } from "@/lib/DatabaseService";
...
export default function PrintCenter() {
  ...
  const printPeriodReport = async () => {
    ...
    try {
      const [other, periodMembers] = await Promise.all([getOtherIncome(selectedMonth, selectedYear), Promise.resolve(members.filter((member) => member.month === selectedMonth && member.year === selectedYear))]);
      const doc = new jsPDF(); doc.setFontSize(16); doc.text(`${settings.name} — Monthly Report`, 14, 18); doc.setFontSize(10); doc.setTextColor(120); doc.text(`${MONTHS[selectedMonth - 1]} ${selectedYear} · Member report first`, 14, 25);
      autoTable(doc, { startY: 32, head: [["#", "Member", "Phone", "Due", "Received", "Pending", "Status", "Mode"]], body: periodMembers.length ? periodMembers.map((member, index) => [index + 1, member.name, member.phone, formatPdfAmount(member.amount), formatPdfAmount(member.amount_paid), formatPdfAmount(member.amount_pending), member.status, (member.payment_mode ?? "cash") === "account" ? "Account" : "Cash"]) : [["No member contribution records for this period.", "", "", "", "", "", "", ""]], headStyles: { fillColor: [20, 120, 90] }, styles: { fontSize: 8 } });
      ...
      const fridayEnd = (doc as jsPDF & { lastAutoTable?: { finalY: number } }).lastAutoTable?.finalY ?? 45; doc.setTextColor(20); doc.text(`Friday Total: ${formatPdfAmount(other.fridayTotal)}`, 14, fridayEnd + 8);
      ...
      const rentEnd = (doc as jsPDF & { lastAutoTable?: { finalY: number } }).lastAutoTable?.finalY ?? fridayEnd + 30; doc.setTextColor(20); doc.setFontSize(11); doc.text(`Room Rent Total: ${formatPdfAmount(other.roomRentTotal)}`, 14, rentEnd + 8); doc.setFontSize(13); doc.text(`Other Total: ${formatPdfAmount(other.otherTotal)}`, 14, rentEnd + 16); doc.save(`monthly-report-${selectedYear}-${month}.pdf`); toast.success("Monthly report ready");
  ...
  return <AppShell title="Print Center" subtitle="Batch print receipts, member lists, and month-end reports with Other collections">
