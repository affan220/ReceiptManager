...
export default function PrintCenter() {
  ...
  const filtered = useMemo(() => members.filter((m) => {
    if (status === "hold") return m.hold && (!search || `${m.name} ${m.phone} ${m.payment_mode ?? "cash"} ${m.voucher_number ?? ""}`.toLowerCase().includes(search.toLowerCase()));
    if (status !== "all" && m.status !== status) return false;
    if (search && !`${m.name} ${m.phone} ${m.payment_mode ?? "cash"} ${m.voucher_number ?? ""}`.toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  ...
  const printList = () => {
    ...
    autoTable(doc, {
      startY: 32,
      head: [["#", "Name", "Phone", "Amount", "Status", "Payment Mode", "Payment Date", "Voucher", "Period"]],
      body: list.map((m, i) => [
        i + 1,
        m.name,
        m.phone,
    ...
    <AppShell title="Print Center" subtitle="Batch print receipts and member lists">
      <div className="card-surface p-4 mb-4 flex flex-wrap items-center gap-3">
        <div className="relative min-w-[200px] flex-1">
          ...
          <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by name, phone, voucher, cash or account..." className="pl-9" />
        </div>
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-muted-foreground truncate">
                  {settings.currency}{m.amount} · {MONTHS[m.month - 1]} {m.year} · {m.status} · {(m.payment_mode ?? "cash") === "account" ? "Account" : "Cash"}{m.voucher_number ? ` · ${m.voucher_number}` : ""}
                </p>
              </div>
            </button>
          );
